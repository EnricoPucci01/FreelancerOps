            ...
            <table style='width:100%; height:100%; border-spacing:0;  border-collapse: collapse;'>
                <tr>
                    <td style='width: 1%; white-space: nowrap;'>
                        ...
                        @else
                        <img src={{asset("storage/profilePic/$dataProfil[foto]")}} style="width: 200px; height: 240px;" class="img-thumbnail">
                        @endif

                    </td>
                    <td style='width: 500px; vertical-align: top; text-align: left;'>
                        <div><h5 class="card-title ml-3">{{$dataCust['nama']}}</h5></div>
                        <div>
                    ...
                    <td style='width: 14%;vertical-align: top; text-align: left;'>
                        @if ($role=='f')
                            <a href={{url('/loadEditProfil/'.session()->get('cust_id'))}} class='btn btn-info' style='margin-left:20%'>Edit Profile</a>
                        @endif
                    </td>
                </tr>
            <tr>
                <td style="width: 1%; white-space: nowrap;">
                    <ul class="list-group list-group-flush">
                         <li class="list-group-item"><i class="bi bi-telephone-fill">{{$dataCust['nomorhp']}}</i></li>
                         <li class="list-group-item"><i class="bi bi-envelope">{{$dataCust['email']}}</i></li>
                         <li class="list-group-item"><i class="bi bi-house-door">{{$dataCust['tempat_lahir']}}</i></li>
                      </ul>
                </td>
