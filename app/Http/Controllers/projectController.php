...
class projectController extends Controller
{
    ...
    public function submitPostProject(Request $request){
        ...
        $postProject->nama_proyek= $request->input('name_project');
        $postProject->desc_proyek= $request->input('desc_project');
        ...
        $postProject->tipe_proyek=$request->input('tipe_proyek');
        $postProject->deadline= $request->input('deadline');
        $postProject->kategorijob_id=$request->input('kategorijob_project');
        $postProject->save();
        ...
        if($postProject){
            ...
            foreach($request->input('kategori_project') as $kategoriId){
                $tag= new tag();
            ...
            if($request->input('tipe_proyek')=="magang"){
                for($i=0;$i<(int)$request->input('hid_val'); $i++){
                    ...
                    $modul= new modul();
                    $modul->proyek_id=$id;
                    $modul->title=$modulMagangArr[$i]['nama_modul'];
                    $modul->deskripsi_modul=$modulMagangArr[$i]['deskripsi_modul'];
                    $modul->bayaran=0;
                    $modul->bayaran_min=$modulMagangArr[$i]['bayaran1'];
                    $modul->bayaran_max=$modulMagangArr[$i]['bayaran2'];
                    $modul->status='not taken';
                    $modul->end=$modulMagangArr[$i]['deadline_modul'];
                    $modul->save();
                }
            }else{
                for($i=0;$i<(int)$request->input('hid_val'); $i++){
                    ...
                    $modul= new modul();
                    $modul->proyek_id=$id;
                    $modul->title=$modulArr[$i]['nama_modul'];
                    $modul->deskripsi_modul=$modulArr[$i]['deskripsi_modul'];
                    $modul->bayaran=$modulArr[$i]['bayaran'];
                    $modul->status='not taken';
                    $modul->end=$modulArr[$i]['deadline_modul'];
                    $modul->save();
                }
            }
    ...
    public function cariProyek(Request $request){
        ...
        if($request->input('kategori_browse')!=NULL){
            ...
            $listProyek=proyek::where('tipe_proyek',$request->tipe_proyek)->
            where('nama_proyek','like','%'.$request->searchProyek.'%')->paginate(5)->appends($request->all());
        }
    ...
    public function loadProgress($modulId,$modulTakenId){
        ...
        $dataProgress=progress::where('modul_id',$modulId)->get();
